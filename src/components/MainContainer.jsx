...
const MainContainer = () => {
    ...
    const [exUrl, setExUrl] = useState(dataBaseProducts)
    ...
    const filterHandler =  (value) =>{   
        if(value === "Barchasi"){
            setDataBase(dataBaseProducts)
        }
        else{
            const productFilter = exUrl.filter(elem => {
                return elem.category === value
            })
            setExUrl(productFilter)
        }
    }
    ...
    const searchHandler = (value) =>{
        const searchFilter = exUrl.filter(elem => {
            return elem.title.toLowerCase().includes(value.toLowerCase())
        })
        setExUrl(searchFilter)
    }
  return (
    <>
       <div>
            <div>
                ...
                <Navbar searchHandler={searchHandler} setIsOpen={setIsOpen} state={state} mx={mx}/>
                
                {/* <div className=' mt-10 mb-20 '>
                {/* <Carusel /> */}
                {/* </div>  */}

            <div className={`flex md:flex-row md:gap-0 gap-4 flex-col justify-between items-center ${mx} mt-10`}>
                ...
                <input  type="text" onChange={(e) => searchHandler(e.target.value)} className='rounded-lg border border-black w-full lg:w-[300px] px-5 py-2' placeholder='Mahsulot qidiring...'/>
            </div>
